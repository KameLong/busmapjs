...
import {ApiRoute, ApiStation} from "./js/a_hanyou";
...
export async function f_busmap() {
	...
	//色々調整
	const busmapData=f_offset_polyline(klAPI,f_zoom_ratio(c_zoom_level));

	//leafletで表示
	//線
	for (let routeID in busmapData.route) {
		busmapData.route[routeID].l_polyline = L.polyline(busmapData.route[routeID].polyline, {"color": busmapData.route[routeID].color, "weight": 1}).addTo(l_map);
	}
	//駅
	for (let i1 in busmapData.route) {
		busmapData.route[i1].l_points = [];
		for (let i2 = 0; i2 < busmapData.route[i1].points.length; i2++) {
			busmapData.route[i1].l_points[i2] = {};
			busmapData.route[i1].l_points[i2]["l_circleMarker"] = L.circleMarker(busmapData.route[i1].points[i2]["latlon"], {"color": "#000000", "fillColor": "#c0c0c0", "fillOpacity": 1, "radius": 4, "weight": 2, "opacity": 1}).addTo(l_map);
		}
	}
	//駅名
	for (let i1 in busmapData.station) {
		L.marker([busmapData.station[i1]["lat"], busmapData.station[i1]["lon"]], {"icon": L.divIcon({"html": busmapData.station[i1]["name"], className: "className", iconSize: [256, 16], iconAnchor: [-4, -4]})}).addTo(l_map);
	}
	f_zoom();
	...
	function f_zoom() {
		f_offset_polyline(busmapData, f_zoom_ratio(l_map.getZoom()));
		//線
		for (let i1 in busmapData.route) {
			busmapData.route[i1].l_polyline.setLatLngs(busmapData.route[i1].polyline);
		}
		//駅
		for (let i1 in busmapData.route) {
			for (let i2 = 0; i2 < busmapData.route[i1].points.length; i2++) {
				busmapData.route[i1].l_points[i2]["l_circleMarker"].setLatLng(busmapData.route[i1].points[i2]["latlon"]);
			}
		}
	}
...
function f_offset_polyline(a_data:KLAPI, a_zoom_ratio:number):BusMapData { //a_zoom_ratioは今のところ無効
